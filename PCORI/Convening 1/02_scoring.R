...
#' Compute per-item priority metrics for one group.
#'
#' @param data         Data frame (already filtered to one group).
...
calc_metrics <- function(data, items, domain_name, group_label) {
  data %>%
    select(all_of(items)) %>%
...
#' Score all domains for both PWA and FMC groups.
#'
#' @param dat  Output of load_and_clean().
#' @return A tibble with item-level results for both groups, plus labels
#'         and known/new metadata.
score_all <- function(dat) {
  results_pwa <- map2_df(dat$domains, names(dat$domains),
                         ~ calc_metrics(dat$df_pwa, .x, .y, "PWA"))
  results_fmc <- map2_df(dat$domains, names(dat$domains),
                         ~ calc_metrics(dat$df_fmc, .x, .y, "FMC"))

  bind_rows(results_pwa, results_fmc) %>%
    mutate(
...
#' Extract the top-N items by priority score for a single group.
#'
#' @param results  Item-level results tibble (one group).
